    ...
    { 
        ...
        private void btnLogin_Click(object sender, EventArgs e)
        {
            ...
            MySqlCommand command = new MySqlCommand(
                "SELECT * FROM `users` WHERE  username = @usn, password = @pass and role = @role  " +
                "UNION SELECT username, password, type.Name FROM `users` INNER JOIN `type` on users.role = type.Number", db.getConnection());
